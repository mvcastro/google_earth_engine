...
function list2Dict(dict1, dict2){
  ...
  dict2 = ee.Algorithms.If(grupo.eq(0), 
                dict2.set('group0Sum', dict1.get('sum')),
            ee.Algorithms.If(grupo.eq(1),
                dict2.set('group1Sum', dict1.get('sum')),
            ee.Algorithms.If(grupo.eq(2),
                dict2.set('group2Sum', dict1.get('sum')), 
              null)));
  return dict2;
}
...
function estat(img, lista) {
  var newFeatCol = ee.FeatureCollection(featCollec).map(function(feat) {
    ...
    var grupo0 = ee.Algorithms.If(newDict.contains('group0Sum'), newDict.get('group0Sum'), 0);
    var grupo1 = ee.Algorithms.If(newDict.contains('group1Sum'), newDict.get('group1Sum'), 0);
    var grupo2 = ee.Algorithms.If(newDict.contains('group2Sum'), newDict.get('group2Sum'), 0);
    
    feat =  feat.set('group0Sum', grupo0)
                .set('group1Sum', grupo1)
                .set('group2Sum', grupo2)
